...
- 基金自选
  - 股票 / 基金分段切换
  - 基金名称 / 代码搜索，只支持场外开放式基金
  - 添加、删除、右键排序和拖拽排序
  - 展示基金名称、代码、估算净值、估算涨跌幅、单位净值、净值日期、估算更新时间
  - 排除 ETF、LOF、场内基金、封闭式基金和股票行情接口里的基金
...
这些接口并非由本项目维护，返回结构、可用性、频率限制和估算更新时间都可能变化。基金估算按场外开放式基金的估算净值 / 估算涨跌幅理解，不等同于交易所逐笔价格。基金搜索会过滤 ETF、LOF、场内和封闭式品种；接口异常时，基金批量刷新会保留已存在的旧数据，单条失败不阻塞其他基金。
