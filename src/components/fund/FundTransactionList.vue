<template>
  <section class="transaction-dialog" role="dialog" aria-label="基金交易记录">
    ...
    <div v-else class="record-empty"><span>⇄</span><strong>暂无交易记录</strong><p>首次加仓或修改持仓后会在这里形成账本</p></div>
...
function transactionIcon(type: FundTransaction['type']): string {
  return { buy: '+', sell: '−', adjustment: '↺' }[type]
}
...
.dialog-head small { margin-left: 8px; color: var(--text-muted); font-size: 8px; }
...
.record-icon.baseline { color: var(--text-muted); background: var(--card-bg); font-size: 9px; }
.record-main { min-width: 0; }
.record-title { display: flex; align-items: center; gap: 7px; }
.record-title strong { font-size: 10px; }
.record-title span { color: var(--text-muted); font-size: 8px; }
.record-main p { overflow: hidden; margin-top: 3px; color: var(--text-secondary); text-overflow: ellipsis; white-space: nowrap; font-size: 9px; }
.record-meta { display: flex; gap: 10px; margin-top: 4px; color: var(--text-muted); font-size: 8px; }
.record-actions { display: flex; gap: 3px; }
.record-actions button { padding: 4px 6px; border: 0; border-radius: 5px; color: #8fb0ff; background: var(--card-bg); font: inherit; font-size: 8px; cursor: pointer; }
.record-actions button:hover { background: var(--card-bg-hover); }
.record-actions button.danger { color: #f28b82; }
.boundary-chip { padding: 3px 5px; border-radius: 4px; color: var(--text-muted); background: var(--card-bg); font-size: 7px; }
.baseline { opacity: .76; }
...
.record-empty p { margin-top: 4px; font-size: 8px; }
...
footer span { color: var(--text-muted); font-size: 8px; }
footer button { min-width: 62px; height: 27px; border: 0; border-radius: 7px; color: white; background: #5b82ed; font: inherit; font-size: 9px; cursor: pointer; }
</style>
